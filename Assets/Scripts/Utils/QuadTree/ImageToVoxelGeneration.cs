...
public class ImageToVoxelGeneration : MonoBehaviour
{
    ...
    void Start()
    {
        if (ArrayTerrain.isActiveAndEnabled)
            Generate(ArrayTerrain);
    ...
    private void Generate(ArrayDestructibleTerrain terrain)
    {
    ...
    private void Generate(DestructibleTerrain terrain)
    {
        int cells = (int) Mathf.Pow(2,terrain.depth);
